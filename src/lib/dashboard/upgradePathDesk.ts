...
function pathPicked(input: UpgradePathInput): boolean {
  ...
  if (user.role === 'artist' || user.role === 'editor' || user.role === 'super_editor') return true
...
export function buildUpgradeTiers(input: UpgradePathInput): UpgradeTier[] {
  ...
  if (user.role === 'editor' || user.role === 'super_editor') {
...
export function buildUpgradeRoadmap(input: UpgradePathInput): UpgradeRoadmapStep[] {
  ...
  } else if (user.role === 'editor' || user.role === 'super_editor') {
    steps.push({
      key: 'editor-desk',
  ...
  if (!artistProfile && !user.dashboardPersona && user.role === 'member') {
    steps[3].state = 'todo'
    steps[4].state = 'todo'
    steps[5].state = 'todo'
    steps[6].state = 'todo'
...
export function buildUpgradeNextStep(input: UpgradePathInput): UpgradeNextStep {
  ...
  if (!artistProfile && user.role === 'member') {
    return {
...
export function buildUpgradePathSnapshot(input: UpgradePathInput): UpgradePathSnapshot {
  ...
  const isEditor = input.user.role === 'editor' || input.user.role === 'super_editor'
  const isArtist = input.user.role === 'artist' || Boolean(input.artistProfile)
...
export function filterQuickActions(
  ...
  return actions.filter((action) => {
    if (action.label === 'Become an Artist' && (snapshot.isArtist || user.role === 'artist')) {
      return false
    }
