-- Facebook-style feed posts: optional photo on spins and drops.
-- A drop may now be photo-only (no text) as long as it has an image.

alter table public.community_posts
  add column if not exists image_url text;

-- Relax: a drop is valid with body (1-280) OR a photo.
alter table public.community_posts
  drop constraint if exists community_posts_drop_requires_body;
...
-- ── Recreate community_feed with image_url ─────────────────────────────
...
-- ── Recreate community_posts_by_handle with image_url ──────────────────
