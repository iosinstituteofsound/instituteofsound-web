...
import { AccessControlPanel } from '@/components/authz/AccessControlPanel'
import { LayoutPicker } from '@/components/authz/LayoutPicker'
import { ResourceGate } from '@/components/auth/ResourceGate'
...
export default function EditorDashboardPage() {
  const { user, authorization, logout, mode, isSuperEditor, refreshUser } = useAuth()
  ...
  const deskBody = (
    <>
        ...
        {authorization?.availableLayouts && authorization.availableLayouts.length > 1 && (
          <LayoutPicker
            layouts={authorization.availableLayouts}
            preferredLayoutId={authorization.preferredLayoutId}
          />
        )}
        ...
        ) : tab === 'access_control' && isSuperEditor ? (
          <ResourceGate resource="desk.access_control">
            <AccessControlPanel />
          </ResourceGate>
        ) : loadingData &&
          tab !== 'write' &&
