...
import { hasArtistAccess, hasEditorialAccess, isSuperAdmin } from '@/lib/auth/roles'
...
export default function SubmissionsPage() {
  ...
  if (user && hasArtistAccess(user.authorization)) {
    return <Navigate to="/artist/dashboard" replace />
  }

  if (user && isSuperAdmin(user.authorization)) {
    ...
    <div className="section-padding pt-32 min-h-screen">
      <div className="max-w-xl mx-auto text-center">
        ...
        <p className="text-xs text-muted mt-8 max-w-md mx-auto">
          ...
          . Desk access is assigned by Super Admin only.
        </p>
      </div>
    </div>
  )
