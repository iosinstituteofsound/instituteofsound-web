...
export function EditorProfilePanel({ user, onSaved }: EditorProfilePanelProps) {
  ...
  const usernameError = username
    ? validateUsername(username, { isSuperAdmin: user.authorization?.isSuperAdmin })
    : null
      ...
      <div>
        ...
        {usernameError ? (
          ...
          <p className="text-xs text-muted mt-1">
            ...
            {isSuperAdmin(user.authorization) && (
              <span className="block mt-1 text-foreground/80">
                Super Admin: you can claim brand handles such as <strong>@ios</strong>.
              </span>
            )}
          </p>
        )}
